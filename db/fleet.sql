...
insert  into `assigneemaster`(`assigneeID`,`companyID`,`locationID`,`firstname`,`lastname`,`gender`,`age`,`contactNo1`,`contactNo2`,`address`,`costCenter`,`immediateHead`,`emailAddress`,`department`,`attachment`,`licenseNo`,`licenseRegistrationDate`,`expirationDate`,`licenseAddress`,`createdBy`,`createdDate`,`modifiedBy`,`modifiedDate`,`status`) values ('ASS00000009','COM0001','LOC0001','b','b','M',30,'123','123','b','b','alladinx','rpcastanares@fastgroup.biz',NULL,'cflogo.gif','123',NULL,'2016-08-31 00:00:00','b','alladinx','2015-08-24 07:57:02',NULL,NULL,1),('ASS00000008','COM0001','LOC0001','A','A','M',22,'123456789','987654321','a','a','noel rodriguez','rpcastanares@fastgroup.biz','IT','gaskull_t-shirt.png','a','2014-07-24 00:00:00','2015-07-24 00:00:00','a','alladinx','2015-07-03 09:08:14','alladinx','2015-10-15 02:44:56',1);
...
insert  into `controlnomaster`(`id`,`description`,`type`,`code`,`lastDigit`,`noOfDigit`,`createdBy`,`createdDate`,`modifiedBy`,`modifiedDate`,`status`) values (1,'CATEGORY','category','CAT',6,3,'','2015-06-29 02:53:37','alladinx','2015-10-01 04:48:19',1),(2,'COMPANY','company','COM',3,4,'','2015-06-29 02:54:16','alladinx','2015-10-01 04:48:29',1),(3,'CUSTOMER','customer','CUS',0,8,'','2015-06-29 02:55:03','','2015-06-29 03:13:16',1),(4,'EMPLOYEE','employee','EMP',0,8,'','2015-06-29 02:59:15',NULL,NULL,1),(5,'EQUIPMENT','equipment','EQ',8,8,'','2015-06-29 03:11:51','alladinx','2015-07-03 09:01:18',1),(6,'MAKE','make','MAK',10,3,'','2015-06-29 03:12:12',NULL,NULL,1),(7,'PARTS','parts','PAR',6,8,'','2015-06-29 03:12:38','alladinx','2015-07-09 03:30:56',1),(8,'MODEL','model','MOD',3,6,'','2015-06-29 03:13:56','','2015-07-01 03:23:56',1),(9,'LOCATION','location','LOC',2,4,'','2015-06-29 03:14:16','','2015-06-30 07:27:47',1),(10,'WORK ORDER','work_order','WO',14,8,'','2015-06-29 03:14:34','alladinx','2015-09-03 06:31:48',1),(11,'PO RECEIVING','po_receiving','PO',1,8,'','2015-06-29 03:14:59','alladinx','2015-09-03 06:31:56',1),(12,'MENUS','menu','M',27,4,'','2015-06-29 03:15:22',NULL,NULL,1),(13,'REMINDERS','reminder','REM',0,8,'','2015-06-29 03:15:42','alladinx','2015-09-04 04:07:28',1),(14,'SUPPLIER','supplier','SUP',2,8,'','2015-06-29 03:16:07',NULL,NULL,1),(15,'ASSIGNEE','assignee','ASS',9,8,'','2015-06-29 03:16:32',NULL,NULL,1),(16,'SERVICE TYPE','service_type','ST',2,3,'','2015-06-29 03:17:00',NULL,NULL,1),(18,'ASSIGNEE EQUIPMENT','assignee_equipment','AE',8,8,'alladinx','2015-09-20 08:40:41','alladinx','2015-09-20 08:45:48',1),(19,'USER ASSIGNEE','user_assignee','UA',5,8,'alladinx','2015-09-20 10:57:20','alladinx','2015-09-20 11:01:35',1),(20,'ASSIGNEE COMPANY','assignee_company','AC',5,8,'alladinx','2015-09-20 11:01:26',NULL,NULL,1),(21,'INVOICING','invoicing','INV',1,8,'alladinx','2015-10-01 04:49:52',NULL,NULL,1);
...
insert  into `workorderdetail`(`woReferenceNo`,`partsID`,`partsName`,`partsPrice`,`qty`,`seqNo`) values ('WO00000010','PAR00000002','asd','250.00',2,1),('WO00000014','PAR00000001',NULL,'150.00',4,1),('WO00000014','PAR00000002',NULL,'400.00',1,2),('WO00000014','PAR00000005',NULL,'200.00',1,3);
...
CREATE TABLE `workordermaster` (
  ...
  `variance` varchar(10) DEFAULT NULL,
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_assigneemaster` AS (select `assigneemaster`.`assigneeID` AS `assigneeID`,`assigneemaster`.`companyID` AS `companyID`,`companymaster`.`companyName` AS `companyName`,`assigneemaster`.`locationID` AS `locationID`,`locationmaster`.`locationName` AS `locationName`,`assigneemaster`.`firstname` AS `firstname`,`assigneemaster`.`lastname` AS `lastname`,concat(`assigneemaster`.`firstname`,_latin1' ',`assigneemaster`.`lastname`) AS `assigneeName`,`assigneemaster`.`gender` AS `gender`,`assigneemaster`.`age` AS `age`,`assigneemaster`.`contactNo1` AS `contactNo1`,`assigneemaster`.`contactNo2` AS `contactNo2`,`assigneemaster`.`address` AS `address`,`assigneemaster`.`costCenter` AS `costCenter`,`assigneemaster`.`immediateHead` AS `immediateHead`,`assigneemaster`.`emailAddress` AS `emailAddress`,`assigneemaster`.`department` AS `department`,`assigneemaster`.`attachment` AS `attachment`,`assigneemaster`.`licenseNo` AS `licenseNo`,`assigneemaster`.`licenseRegistrationDate` AS `licenseRegistrationDate`,`assigneemaster`.`expirationDate` AS `expirationDate`,`assigneemaster`.`licenseAddress` AS `licenseAddress`,`assigneemaster`.`createdBy` AS `createdBy`,`assigneemaster`.`createdDate` AS `createdDate`,`assigneemaster`.`modifiedBy` AS `modifiedBy`,`assigneemaster`.`modifiedDate` AS `modifiedDate`,`assigneemaster`.`status` AS `status`,(case when (`assigneemaster`.`status` = '0') then 'INACTIVE' when (`assigneemaster`.`status` = '1') then 'ACTIVE' end) AS `statusDesc` from ((`assigneemaster` join `companymaster` on((`companymaster`.`companyID` = `assigneemaster`.`companyID`))) join `locationmaster` on((`locationmaster`.`locationID` = `assigneemaster`.`locationID`)))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_workorderdetail` AS (select `workorderdetail`.`woReferenceNo` AS `woReferenceNo`,`workorderdetail`.`partsID` AS `partsID`,`partsmaster`.`description` AS `partsName`,`workorderdetail`.`partsPrice` AS `partsPrice`,`workorderdetail`.`qty` AS `qty`,`workorderdetail`.`seqNo` AS `seqNo` from (`workorderdetail` join `partsmaster` on((`partsmaster`.`partsID` = `workorderdetail`.`partsID`)))) */;
